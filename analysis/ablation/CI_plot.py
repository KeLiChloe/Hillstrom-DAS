...
# Y轴只显示3个 comparator，并把显著性标在Y轴上（避免和点重叠）
star_map = {}  # {(Comparator, Variant): P_Star}
for _, r in stats_df.iterrows():
    star_map[(r["Comparator"], r["Variant"])] = r["P_Star"]
...
for comp in comp_order:
    s_non = fmt_star(star_map.get((comp, "Non-DAMS"), None))
    s_dam = fmt_star(star_map.get((comp, "DAMS"), None))
    # 两行：上 Non-DAMS，下 DAMS（与你的offset方向一致）
    ytick_labels.append(f"{comp} — Non-DAMS{s_non}\n{comp} — DAMS{s_dam}")

ax.set_yticks(range(len(comp_order)))
ax.set_yticklabels(ytick_labels, fontweight="bold", fontsize=14, rotation=0)
...
legend_handles = [
    Line2D([0], [0], color=COLOR_NON_DAMS, linestyle=LINESTYLE_NON_DAMS, marker="o",
           linewidth=2, markersize=8, label="Standard"),
    Line2D([0], [0], color=COLOR_DAMS, linestyle=LINESTYLE_DAMS, marker="o",
           linewidth=2, markersize=8, label="DAMS"),
